...
class TRPO:
    ...
    def hessian_matrix_vector_product(self, states, old_action_dists, vector, damping=0.1):
        ...
        return grad2_vector.detach() + damping * vector.detach()  # 加上阻尼项,使得计算更稳定z
...
def train_discrete():
    writer = SummaryWriter(log_dir='logs/trpo/discrete')
    # 设置随机种子，确保实验可复现
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
    ...
    torch.save(agent.actor.state_dict(), f"{path}/discrete_actor_{date_str}.pth")
    torch.save(agent.critic.state_dict(), f"{path}/discrete_critic_{date_str}.pth")
...
def train_continuous():
    # 训练处理连续动作的TRPO算法
    writer = SummaryWriter(log_dir='logs/trpo/continuous')
    # 设置随机种子，确保实验可复现
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    date_str = datetime.now().strftime("%Y%m%d")
    torch.save(agent.actor.state_dict(), f"{path}/continuous_actor_{date_str}.pth")
    torch.save(agent.critic.state_dict(), f"{path}/continuous_critic_{date_str}.pth")
    ...
    if args.test:
        ...
        if args.env_id == 'Pendulum-v1':
            ...
            agent = TRPOContinuous(hidden_dim, env.observation_space, env.action_space, lmbda,
                    kl_constraint, alpha, critic_lr, gamma, device)
            path = os.path.join('model', '_'.join(["TRPO", args.env_id]))
            agent.actor.load_state_dict(torch.load(f"{path}/continuous_actor_{date_str}.pth"))
            agent.critic.load_state_dict(torch.load(f"{path}/continuous_critic_{date_str}.pth"))
        else:
            alpha = 0.5
            hidden_dim = 128
            gamma = 0.98
            ...
            path = os.path.join('model', '_'.join(["TRPO", args.env_id]))
            agent.actor.load_state_dict(torch.load(f"{path}/discrete_actor_{date_str}.pth"))
            agent.critic.load_state_dict(torch.load(f"{path}/discrete_critic_{date_str}.pth"))
